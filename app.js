/* =========================================================
   English Library - app.js V2 升級整合版
========================================================= */
...
function loadVoices() {
  ...
  for (const name of preferred) {
    const found = englishVoices.find(function (voice) {
...
function setSpeechRate(rate) {
  ...
  speechRate = Number(value.toFixed(1));
...
function updateSpeedHighlight() {
  ...
  buttons.forEach(function (button) {
    ...
    if (Math.abs(rate - speechRate) < 0.001) {
...
function updateIntervalHighlight() {
  ...
  buttons.forEach(function (button) {
    ...
    if (interval === playbackInterval) {
      button.classList.add("active-interval");
    } else {
      button.classList.remove("active-interval");
    }
  });
}
...
function updatePauseButton() {
  ...
  button.textContent = isPaused ? "▶ 繼續" : "⏸ 暫停";
}
...
function checkDuplicateOnInput() {
  ...
  const match = sentences.find(function (item) {
...
async function translateNewSentence() {
  ...
  if (message) message.textContent = "正在翻譯...";
  ...
  try {
    ...
    chineseInput.value = result.chinese || "";

    if (message) message.textContent = "✅ 翻譯完成";
  } catch (err) {
...
async function addSentence() {
  ...
  try {
    ...
    const newIndex = sentences.findIndex(function (item) {
...
window.speak = speak;
window.repeatSentence = repeatSentence; // 匯出單句重播
...
window.checkDuplicateOnInput = checkDuplicateOnInput; // 匯出即時重複檢查
